...
<div align="center" style="clear: both;">
	...
	<!-- 시작 페이지가 pagePerBlock보다 크면 앞에 보여줄 것이 있다 -->
		<c:if test="${startPage > pagePerBlock }">
			<li><a href="areaPlaceList.do?pageNum=1&place_area=${place_area }&keyword=${place.keyword}">
				<span class="glyphicon glyphicon-backward"></span></a></li>
			<li><a href="areaPlaceList.do?pageNum=${startPage-1}&place_area=${place_area }&keyword=${place.keyword}">
				<span class="glyphicon glyphicon-triangle-left"></span></a></li>
		</c:if>
		<c:forEach var="i" begin="${startPage }" end="${endPage }">
			<c:if test="${currentPage==i }">
				<li class="active"><a href="areaPlaceList.do?pageNum=${i}&place_area=${place_area }&keyword=${place.keyword}">${i}</a></li>
			</c:if>
			<c:if test="${currentPage!=i }">
				<li><a href="areaPlaceList.do?pageNum=${i}&place_area=${place_area }&keyword=${place.keyword}">${i}</a></li>
			</c:if>
		</c:forEach>
		<!-- 보여줄 것이 남아있는 경우에는 endPage보다 totalPage가 큰경우 -->
		<c:if test="${endPage < totalPage }">
			<li><a href="areaPlaceList.do?pageNum=${endPage+1}&place_area=${place_area }&keyword=${place.keyword}">
				<span class="glyphicon glyphicon-triangle-right"></span></a></li>
			<li><a href="areaPlaceList.do?pageNum=${totalPage}&place_area=${place_area }&keyword=${place.keyword}">
