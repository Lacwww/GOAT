...
public class NoticeReplyController {
	...
	public String rInsert(NoticeReply nr, HttpSession session) {
		...
		System.out.println("nor_num"+nor_num);
		...
		if(nor_num != 0) { // 댓글에 댓글을 달 때
			...
			if(nr1.getNor_re_step() == 0 && nr1.getNor_re_level() == 0) {
				...
				int maxStep = nrs.maxStep(); // 댓글중에서 새로운 댓글 달때 맨 밑으로 가기 위해서
				...
				System.out.println("nor_re_step:"+nr1.getNor_re_step());
				nr.setNor_re_level(nr1.getNor_re_level());
				System.out.println("nor_re_level:"+nr1.getNor_re_level());
